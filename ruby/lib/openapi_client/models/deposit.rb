  ...
  class Deposit
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Deposit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Deposit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @received_timestamp.nil?
        invalid_properties.push('invalid value for "received_timestamp", received_timestamp cannot be nil.')
      end
    ...
    def valid?
      ...
      state_validator = EnumAttributeValidator.new('String', ["pending", "completed", "rejected", "replaced"])
      return false unless state_validator.valid?(@state)
      return false if @received_timestamp.nil?
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["pending", "completed", "rejected", "replaced"])
    ...
    def hash
      [updated_timestamp, state, received_timestamp, currency, address, amount, transaction_id].hash
    end
